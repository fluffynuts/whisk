...
var APP_NAME = '';  // please define your app name here
var DEBUG = false;
...
function cordovaIncluder() {
    ...
    var stream = through.obj(function(file, enc, cb) {
        if (file.isBuffer()) {
            var newContents = file.contents.toString().replace(/<head>/, "<head><script type=\"text/javascript\" src=\"cordova.js\"></script>");
            file.contents = new Buffer(newContents);
        } else if (file.isStream()) {
            console.log("file is a stream:" + file.path);
        }
        this.push(file);
        return cb();
    });
...
gulp.task('cordova-build-wp8', ['default', 'copy-res'], $.shell.task([
    'cordova build wp8'
], { cwd: 'dist' } ));

gulp.task('cordova-build-android', ['default', 'copy-res'], $.shell.task([
    'cordova build android'
], { cwd: 'dist' } ));

gulp.task('cordova-build-', ['default', 'copy-res'], function(cb) {
    runSequence('cordova-build-android', 'cordova-build-wp8', cb);
});

gulp.task('push', ['cordova-build-android'], $.shell.task([
    'adb install -r ' + APP_NAME + '-debug.apk'
], { cwd: 'dist/platforms/android/ant-build' }));

gulp.task('win8', ['default', 'copy-res'], $.shell.task([
